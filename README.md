...
[`SPDX-AI-Disclosure`](https://spdx.dev) tag or a tool header (`@cursor-generated`,
...
For code, cryptographic attestation systems are also out of scope, for two distinct reasons:

- **Build/package provenance (SLSA, npm/PyPI Sigstore, `gh attestation verify`, `cosign`)** is
  real and verifiable today, but it answers a different question: it proves an artifact was built
  from a given commit by a given CI workflow, not whether the code was AI-written. It is
  supply-chain integrity, not AI provenance, so it is not a gap here.
- **AI-authorship attestations (Pedigree, ForgeProof, agentdiff)** would be squarely in character
  - they are cryptographic authorship provenance - but each uses a bespoke predicate with
  near-zero adoption and no stable verifier CLI. We only surface a cryptographic claim if we can
  verify it, and we will not reimplement security-critical signature/transparency-log verification
  we could otherwise shell out to. Revisit trigger: if one standardizes (e.g. a recognized in-toto
  predicate type) and ships an official verifier, wire it in by shelling out to that verifier, the
  same way `check` shells out to `c2patool`.
